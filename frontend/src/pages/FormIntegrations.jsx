...
import {
  ...
  Zap, Globe, Check, X, ChevronDown, ChevronUp, Settings,
  Play, AlertTriangle, Clock, ArrowRight, RefreshCw, Trash2, Edit2
} from 'lucide-react'
...
const initialIntegrations = [
  {
    id: 'int-001',
    type: 'slack',
    displayName: 'Growth Team Alerts',
    enabled: true,
    events: ['waitlist.signup.created', 'waitlist.signup.milestone'],
  ...
  },
  {
    id: 'int-002',
    type: 'webhook',
    displayName: 'CRM Sync Webhook',
    enabled: true,
    events: ['waitlist.signup.created', 'waitlist.signup.verified'],
    lastTriggered: '15 min ago',
    failureCount: 0,
    config: { url: 'https://api.internal.acme.com/waitlist-hook' },
  },
  {
    id: 'int-003',
    type: 'mailchimp',
    displayName: 'Marketing List Sync',
    enabled: false,
    events: ['waitlist.signup.verified'],
    lastTriggered: '3 days ago',
    failureCount: 2,
    config: { audience: 'Launch Subscribers' },
  },
]
      ...
      {showAdd && (
        <div className="card-surface p-5 mb-6 animate-slide-up">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setShowAdd(false)} className="text-nexus-500 hover:text-nexus-300"><X size={16} /></button>
          </div>
      ...
      <div className="space-y-3">
        {integrations.length === 0 && (
          <div className="card-surface p-12 text-center">
            ...
            <button onClick={() => setShowAdd(true)} className="btn-secondary mt-4 text-xs">Add Your First Integration</button>
          </div>
        )}
        ...
        {integrations.map(int => {
          ...
          return (
            <div key={int.id} className={`card-surface overflow-hidden transition-all ${int.enabled ? 'border-cyan-glow/8' : 'opacity-60'}`}>
              {/* Header */}
              <div className="flex items-center gap-4 p-4 cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : int.id)}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <div className="w-4 h-4 rounded-full bg-white absolute top-[2px] transition-transform"
                      style={{ width: 16, height: 16, transform: int.enabled ? 'translateX(22px)' : 'translateX(2px)' }} />
                  </button>
              ...
              {isExpanded && (
                <div className="border-t border-cyan-glow/[0.06] p-5 space-y-4 animate-fade-in">
                  ...
                  <div className="flex items-center justify-between pt-2">
                    <div className="flex items-center gap-2">
                      <button onClick={() => sendTest(int.id)}
                        className="btn-secondary text-xs py-1.5 px-3 flex items-center gap-1.5">
                        {testSent === int.id ? <><Check size={12} /> Sent!</> : <><Play size={12} /> Send Test</>}
                      </button>
                      <button className="btn-ghost text-xs flex items-center gap-1.5"><RefreshCw size={12} /> Retry Failed</button>
                    </div>
                    <button onClick={() => removeIntegration(int.id)}
