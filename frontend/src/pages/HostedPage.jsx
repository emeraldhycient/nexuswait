...
import {
  Globe, Eye, Palette, Type, Image, Layout, Monitor, Tablet, Smartphone,
  ArrowLeft, Save, ExternalLink, Copy, Check, Settings, Sparkles,
  AlignLeft, AlignCenter, ChevronDown, Layers, Plus, GripVertical, Trash2
} from 'lucide-react'
...
const sectionTypes = [
  ...
  { id: 'social-proof', label: 'Social Proof', icon: Sparkles },
  { id: 'countdown', label: 'Countdown Timer', icon: Settings },
  { id: 'faq', label: 'FAQ', icon: AlignLeft },
  { id: 'custom-html', label: 'Custom HTML', icon: Type },
]
...
export default function HostedPage() {
  ...
  const update = (key, val) => setPageConfig(c => ({ ...c, [key]: val }))
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-nexus-400 mt-1">
            Design and publish your waitlist landing page.
          </p>
        </div>
            ...
            {activeTab === 'design' && (
              <div className="space-y-5 animate-fade-in">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <input type="color" value={pageConfig.primaryColor} onChange={e => update('primaryColor', e.target.value)}
                      className="w-9 h-9 rounded-lg cursor-pointer border border-nexus-600 bg-transparent" />
                    <input type="text" value={pageConfig.primaryColor} onChange={e => update('primaryColor', e.target.value)}
                ...
                {sections.map(sec => (
                  <div key={sec.id} className="flex items-center gap-2 p-3 rounded-lg bg-nexus-700/15 border border-nexus-700/20">
                    ...
                    <button
                      ...
                      className={`w-8 h-4.5 rounded-full transition-all relative ${sec.enabled ? 'bg-cyan-glow' : 'bg-nexus-600'}`}
                      style={{ width: 32, height: 18 }}
                    >
                      <div className="w-3.5 h-3.5 rounded-full bg-white absolute top-[2px] transition-transform"
                        style={{ width: 14, height: 14, transform: sec.enabled ? 'translateX(16px)' : 'translateX(2px)' }} />
                    </button>
                    <button onClick={() => removeSection(sec.id)} className="text-nexus-600 hover:text-magenta-glow transition-colors">
                      <Trash2 size={13} />
                    </button>
                  </div>
                ))}
                <button className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-lg border border-dashed border-nexus-600 text-xs text-nexus-500 hover:text-cyan-glow hover:border-cyan-glow/30 transition-all">
            ...
            {activeTab === 'seo' && (
              <div className="space-y-4 animate-fade-in">
                ...
                <div>
                  ...
                  <input type="text" value={pageConfig.customDomain} onChange={e => update('customDomain', e.target.value)}
                    className="input-field text-sm" placeholder="waitlist.yoursite.com" />
                ...
                <div>
                  ...
                  <input type="url" value={pageConfig.ogImage} onChange={e => update('ogImage', e.target.value)}
        ...
        <div className="card-surface overflow-hidden">
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-cyan-glow/[0.06]">
            ...
            <div className="flex items-center gap-1.5 px-3 py-1 rounded bg-nexus-900/50 border border-nexus-700/30">
              ...
              <span className="font-mono text-[10px] text-nexus-400">
                {pageConfig.customDomain || `nexuswait.io/${pageConfig.slug}`}
              </span>
            </div>
            <button onClick={handleCopy} className="text-nexus-500 hover:text-cyan-glow transition-colors">
              {copied ? <Check size={13} /> : <Copy size={13} />}
            </button>
          </div>

          <div className="flex justify-center p-4 bg-nexus-900/30 min-h-[500px]">
            <div style={{ width: previewMode === 'desktop' ? '100%' : previewMode === 'tablet' ? '768px' : '375px', maxWidth: '100%' }}
              className="transition-all duration-300">
              {/* Live preview */}
              <div className={`rounded-lg overflow-hidden ${themes.find(t => t.id === selectedTheme)?.preview || ''}`}
                style={{ minHeight: 460 }}>
                <div className="p-8 sm:p-12 text-center flex flex-col items-center justify-center min-h-[460px]">
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-2 w-full max-w-sm">
                    ...
                    <div className="h-10 px-6 rounded-lg flex items-center justify-center text-xs font-bold tracking-wider text-nexus-900"
                      style={{ background: pageConfig.primaryColor, fontFamily: pageConfig.headingFont }}>
                    ...
                    <div className="mt-10 pt-6 border-t border-white/5 w-full">
                      <div className="flex items-center justify-center gap-4 flex-wrap">
                        {sections.filter(s => s.enabled).map(sec => (
                          <span key={sec.id} className="text-[9px] font-mono text-white/20 px-2 py-1 border border-white/5 rounded">
                            {sec.label}
                          </span>
                        ))}
                      </div>
