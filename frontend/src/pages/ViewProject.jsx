...
import {
  ArrowLeft, Users, TrendingUp, Share2, Eye, Copy, ExternalLink,
  Download, Filter, MoreHorizontal, Clock, Mail, ArrowUpRight,
  Pause, Play, Settings, Code, Globe
} from 'lucide-react'

const tabs = ['Overview', 'Subscribers', 'Referrals', 'Settings']

const subscribers = [
  { name: 'Alex Chen', email: 'alex@startup.io', date: 'Mar 11, 2026', source: 'referral', position: 1, referrals: 12 },
  { name: 'Mira Patel', email: 'mira@design.co', date: 'Mar 11, 2026', source: 'direct', position: 2, referrals: 8 },
  { name: 'Jordan Kim', email: 'jordan@dev.xyz', date: 'Mar 10, 2026', source: 'twitter', position: 3, referrals: 5 },
  { name: 'Sam Rivera', email: 'sam@venture.vc', date: 'Mar 10, 2026', source: 'referral', position: 4, referrals: 3 },
  { name: 'Tara Osei', email: 'tara@cloud.dev', date: 'Mar 10, 2026', source: 'producthunt', position: 5, referrals: 2 },
  { name: 'Liam Walsh', email: 'liam@saas.io', date: 'Mar 9, 2026', source: 'direct', position: 6, referrals: 1 },
  { name: 'Ava Tanaka', email: 'ava@agency.jp', date: 'Mar 9, 2026', source: 'referral', position: 7, referrals: 0 },
  { name: 'Diego Morales', email: 'diego@fintech.mx', date: 'Mar 9, 2026', source: 'direct', position: 8, referrals: 0 },
]
    ...
    <div className="animate-fade-in">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-0.5">
              ...
              <button className="text-nexus-600 hover:text-cyan-glow"><Copy size={10} /></button>
            </div>
          </div>
        </div>
        ...
        {stats.map((s, i) => (
          <div key={i} className="card-surface p-4">
            <div className="flex items-center justify-between mb-2">
              <s.icon size={15} className="text-nexus-500" />
          ...
          <div className="card-surface p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2">
                {['7d', '30d', '90d'].map(period => (
                  <button key={period} className="text-[10px] font-mono text-nexus-500 hover:text-cyan-glow px-2 py-1 rounded bg-nexus-700/30 hover:bg-cyan-glow/5 transition-all">
                    {period}
                  </button>
                ))}
              </div>
            ...
            <div className="h-48 flex items-end gap-[3px]">
              {Array.from({ length: 60 }, (_, i) => {
                ...
                const h = Math.min(Math.max(base + Math.random() * 15, 5), 100)
                return (
                  <div
                    key={i}
      ...
      {activeTab === 'Subscribers' && (
        <div className="animate-fade-in">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <button className="btn-ghost text-xs flex items-center gap-1.5"><Filter size={13} /> Filter</button>
              <button className="btn-ghost text-xs flex items-center gap-1.5"><Download size={13} /> Export CSV</button>
            </div>
      ...
      {activeTab === 'Settings' && (
        <div className="animate-fade-in space-y-5 max-w-lg">
          ...
          <div className="pt-6 border-t border-nexus-700/30">
            ...
            <button className="px-4 py-2 rounded-lg border border-magenta-glow/30 text-magenta-glow text-xs font-display font-bold tracking-wider hover:bg-magenta-glow/10 transition-all">
              Delete Project
            </button>
          </div>
        </div>
